...
/*************************************************************************************************/
	...
	if ($arResult["ERROR"] == '')
	{
		...
		if (($_POST['WEB_FORM_ID'] == $arParams['WEB_FORM_ID'] || $_POST['WEB_FORM_ID'] == $arResult['arForm']['SID']) && ($_REQUEST["web_form_submit"] <> '' || $_REQUEST["web_form_apply"] <> ''))
		{
			...
			//$recaptcha = new \ReCaptcha\ReCaptcha(RE_SEC_KEY, new \ReCaptcha\RequestMethod\SocketPost());

			/*$recaptcha = new \ReCaptcha\ReCaptcha(RE_SEC_KEY, new \ReCaptcha\RequestMethod\CurlPost());

			$resp = $recaptcha->verify($_REQUEST['token'], $_SERVER['REMOTE_ADDR']);
			if (!$resp->isSuccess()){
				foreach ($resp->getErrorCodes() as $code) {
					if ($arParams['USE_EXTENDED_ERRORS'] == 'Y')
					{
						$arResult["FORM_ERRORS"]["RECAPTCHA"] = GetMessage("RECAPTCHA_ERROR"). " (код ошибки: ". $code .")";
					}
					else
					{
						$arResult["FORM_ERRORS"] .= '<br>'.GetMessage("RECAPTCHA_ERROR"). " (код ошибки: ". $code .")";
					}
				}
			}*/
			/*google ReCapthcha*/
						...
						if ($arParams["SUCCESS_URL"] <> '')
						{
							if ($arParams['SEF_MODE'] == 'Y')
							{
								LocalRedirect(
									...
									."formresult=".urlencode($arResult["FORM_RESULT"])
									."&tpl=".urlencode($arParams["COMPONENT_TEMPLATE"])
								);
							}
							else
							{
						...
						else
						{
							LocalRedirect(
								$APPLICATION->GetCurPageParam(
									...
									."&RESULT_ID=".$RESULT_ID
									."&formresult=".urlencode($arResult["FORM_RESULT"])
									."&tpl=".$arParams["COMPONENT_TEMPLATE"],
